...
// Ref: https://github.com/LayerZero-Labs/solidity-examples/blob/main/contracts/token/oft/v2/ProxyOFTV2.sol
contract ETHxProxyOFTV2 is BaseOFTV2 {
  ...
  constructor(address _token, uint8 _sharedDecimals, address _lzEndpoint) BaseOFTV2(_sharedDecimals, _lzEndpoint) {
    ...
    require(success, "ETHxProxyOFTV2: failed to get token decimals");
    uint8 decimals = abi.decode(data, (uint8));

    require(_sharedDecimals <= decimals, "ETHxProxyOFTV2: sharedDecimals must be <= decimals");
  ...
  ************************************************************************/
  function _debitFrom(address _from, uint16, bytes32, uint _amount) internal virtual override returns (uint) {
    require(_from == _msgSender(), "ETHxProxyOFTV2: owner is not send caller");
    ...
    require(cap >= outboundAmount, "ETHxProxyOFTV2: outboundAmount overflow");
